...
void START_bit(void)
{
	...
	__delay32( TBUF );	// Wait a few microseconds
	mSCL_HIGH();			// Set SCL line
	__delay32( TBUF );	// Generate bus free time between Stop
							// and Start condition (Tbuf=4.7us min)
	mSDA_LOW();				// Clear SDA line
	__delay32( TBUF );	// Hold time after (Repeated) Start
...
void STOP_bit(void)
{
	...
	__delay32( TBUF );	// Wait a few microseconds
	mSCL_HIGH();			// Set SCL line
	__delay32( TBUF );	// Stop condition setup time(Tsu:sto=4.0us min)
	mSDA_HIGH();			// Set SDA line
}
//*********************************************************************************************
...
void send_bit(unsigned char bit_out)
{
	...
	Nop();							//
	mSCL_HIGH();					// Set SCL line
	__delay32( HIGHLEV );			// High Level of Clock Pulse
	mSCL_LOW();						// Clear SCL line
	__delay32( HIGHLEV );			// Low Level of Clock Pulse
